...
public class Repository<T> : IRepository<T> where T : IDocument
{
    ...
    {
        return PagedList<T>.ToPagedList(_collection.Find(filterExpression), paginationParams.PageNumber, paginationParams.PageSize);
    }

    public Task<PagedList<T>> FilterByAsync(Expression<Func<T, bool>> filterExpression, PaginationParams paginationParams)
    {
        return Task.Run(() => PagedList<T>.ToPagedList(_collection.Find(filterExpression), paginationParams.PageNumber, paginationParams.PageSize));
    }
    ...
    {
        return Task.Run(() => _collection.UpdateMany(filter, update));
    }
}
